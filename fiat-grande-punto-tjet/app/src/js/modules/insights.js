...
(function () {
  ...
  /* d.label here can originate from stored/imported data (a service
     record's year, a budget category name) — everything is placed via
     text:, never interpolated into a markup string, so a crafted
     backup can't inject through this chart. */
  function barChart(title, data) {
    const card = el('div', { class: 'card', style: 'margin-bottom:16px' });
    const max = Math.max.apply(null, data.map(d => d.val).concat([1]));
    const pad = el('div', { class: 'pad' }, [
      el('div', { class: 'mono muted', style: 'font-size:10px;letter-spacing:.14em;text-transform:uppercase;margin-bottom:8px', text: title })
    ]);
    if (!data.length) { pad.appendChild(ui.empty('No data yet.')); }
    else data.forEach(d => {
      const fill = el('div', { style: 'height:100%;width:' + ((d.val / max) * 100) + '%;background:repeating-linear-gradient(-45deg,var(--rosso-deep) 0 8px,var(--rosso) 8px 16px)' });
      pad.appendChild(el('div', { style: 'display:flex;align-items:center;gap:10px;margin:6px 0' }, [
        el('span', { class: 'mono', style: 'width:64px;font-size:10px;color:var(--ink-soft);text-align:right;overflow:hidden;text-overflow:ellipsis;white-space:nowrap', text: d.label }),
        el('div', { style: 'flex:1;background:var(--paper);border:1.5px solid var(--ink);border-radius:3px;height:18px;overflow:hidden' }, [fill]),
        el('span', { class: 'mono', style: 'width:78px;font-size:11px', text: App.util.num(d.val) })
      ]));
    });
  ...
  /* ---- Assistant — rule-based canned answers today, NOT a real AI.
     Labeled and disclaimed honestly (issue: don't present a placeholder
     as if it were live AI). Answer text is built via text:, never
     innerHTML — a fault `code` or budget `cat` below can come from
     imported/stored data, so it must never be interpolated as markup. */
  App.router.register({
    id: 'assistant', label: 'Assistant (rule-based)', group: 'Insights', icon: App.config.icon('assistant'), soon: true, soonLabel: 'DEMO',
    render(view) {
      view.appendChild(ui.secHead('17', 'Maintenance Assistant', 'مساعد الصيانة'));
      view.appendChild(el('div', { class: 'card', style: 'margin-bottom:14px;border-color:var(--amber)' }, [el('div', { class: 'pad', style: 'font-size:11.5px;color:var(--ink-soft)' }, [
        el('b', { style: 'color:var(--amber)', text: '⚠ Not a real AI. ' }),
        el('span', { text: 'These are fixed, rule-based answers written for this demo — nothing is sent to any AI model. A real "AI Maintenance Advisor" (flag ' }),
        el('code', { text: 'aiAdvisor' }),
        el('span', { text: ') is a planned extension point, not implemented yet.' })
      ])]));
      ...
      function answer(q) {
        ...
        else if (/first/i.test(q)) txt = 'Critical open faults: ' + (d.faults.filter(f => !f.solved && f.priority === 'critical').map(f => f.code).join(', ') || 'none') + '. Start with the rear-harness wiring (B1023) + coolant leak + intake hose.';
        ...
        ans.innerHTML = '';
        ans.appendChild(el('div', { class: 'card', style: 'border-color:var(--steel)' }, [
          el('div', { class: 'pad', style: 'font-size:13px;line-height:1.6' }, [
            el('b', { class: 'mono', style: 'color:var(--steel)', text: 'RULE-BASED · ' }),
            el('span', { text: txt })
          ])
        ]));
      }
