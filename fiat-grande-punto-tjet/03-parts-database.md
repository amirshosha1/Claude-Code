# 03 — Master Parts Database

Statuses: `NEEDS ORDERING` · `WAITING QUOTE` · `ORDERED` · `PURCHASED` · `INSTALLED` · `ON HOLD (diagnosis)`
⚠️ All OEM numbers to be **confirmed against VIN in ePER** before ordering (column "Verified" flips to ✅ then).

**Revised 2026-07-12 after service history** — items already fitted moved to INSTALLED with real dates/km/prices from the ledger.

| # | Part | OEM # (candidate) | Verified | Alt./Cross numbers | Chosen brand | Supplier | Qty | Price (EGP) | Status | Warranty | Installed date | Installed km |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 1 | Engine oil 5W-40 C3 (9.55535-S2) | — | — | Selenia K P.E. · Motul 8100 X-cess · Mobil | Selenia/Motul | TBD | 4 L | ~820 (ledger) | NEEDS ORDERING | | | |
| 2 | Oil filter | 55223416 | ⬜ | UFI 23.475.00 · Mann · Mahle OC | UFI/Mann | TBD | 1 | ~130 | NEEDS ORDERING | | | |
| 3 | Air filter | 55192012 | ⬜ | Mann C-series · Mahle | Mann | TBD | 1 | ~250 | NEEDS ORDERING | | | |
| 4 | Cabin filter | via ePER | ⬜ | Mann CU-series · Bosch | Mann | TBD | 1 | 120–150 | NEEDS ORDERING | | | |
| 5 | Spark plugs iridium (OE heat range) | 55212466 | ⬜ | NGK IKR9F8-type · Denso equivalent | NGK | TBD | 4 | ~1,900 set | NEEDS ORDERING — overdue | | | |
| 6 | Brake fluid DOT4 | — | — | ATE · Bosch · TRW | ATE | TBD | 1 L | 250–500 | NEEDS ORDERING (last flush @125k) | | | |
| 7 | Coolant Paraflu UP / G12+ (switch from Mannol) | — | — | Petronas Paraflu UP | Paraflu | TBD | 4–5 L | TBD | SCHEDULED with next belt job | | | |
| 8 | ~~Gearbox oil~~ | — | — | — | — | — | — | — | ✅ DONE @125k (owner confirmed) | | | 125,000 |
| 9 | Wiper blades | — | — | Bosch Aerotwin | Bosch | TBD | 1 pair | ~300 | NEEDS ORDERING | | | |
| 10 | Rear shocks (pair) | via ePER | ⬜ | Sachs · KYB · Monroe | TBD | TBD | 0–2 | owner quote 1,500 | ON HOLD (inspection — likely) | | | |
| 11 | Front suspension items as found | via ePER | ⬜ | TRW · Lemförder · Sachs/KYB · SNR/SKF | TBD | TBD | TBD | TBD | ON HOLD (shake-down, 53k since overhaul) | | | |
| 12 | **Rear discs (pair) + rear pads** | via ePER | ⬜ | Brembo · ATE · TRW | TBD | TBD | 1 set | owner quotes: discs 1,200–1,500 + pads 600–900 | **NEEDS ORDERING — owner confirms worn** | | | |
| 13 | Battery ~60Ah 540CCA | — | — | Varta · Bosch | TBD | TBD | 0–1 | 3,500–5,500 | ON HOLD (CCA test) | | | |
| 14 | Tires | — | — | Michelin · Continental · Hankook | TBD | TBD | 0–4 | TBD | ON HOLD (DOT check) | | | |
| 15 | Ignition coil | 55200486 | ⬜ | Eldor · Bosch · Magneti Marelli | TBD | TBD | 0–4 | TBD | ON HOLD (diagnosis) | | | |
| 16 | Boost solenoid | Pierburg 7.02256.04 cross | ⬜ | Pierburg · Bosch | Pierburg | TBD | 0–1 | TBD | ON HOLD (boost log) | | | |
| 17 | A/C compressor clutch/piston | via ePER | ⬜ | Denso · Valeo | TBD | TBD | 0–1 | owner quote 1,000 | ON HOLD (A/C diagnosis) | | | |
| 18 | Brake hoses ×4 (original 2010 rubber) | via ePER | ⬜ | TRW · ATE | TBD | TBD | 0–4 | 350–700 ea | ON HOLD (inspection) | | | |
| 19 | Timing belt kit + water pump (NEXT cycle) | 71771499 fam. | ⬜ | Gates KP15646XS · SKF VKMC 02390 · INA | Gates | TBD | 1 | ledger 2023: 5,400 all-in | SCHEDULED ~172–180k / Oct 2027 | | | |
| 20 | **Air intake hose (filter→turbo)** | **51822558** | ✅ **confirmed on part (photo)** | genuine (shaped) · adjacent hose stamped 5180472 | Genuine | TBD | 1 | TBD | **NEEDS ORDERING — CONFIRMED TORN** | | | |
| 21 | Expansion tank + cap | 51718005 / cap 55700508 | ⬜ | Behr/Metalcaucho | Genuine | TBD | 1 | TBD | **NEEDS ORDERING — leak CONFIRMED** | | | |
| 22 | Accessory belt + tensioner + idler set | via ePER (measure) | ⬜ | Gates · SKF · INA · Contitech | Gates/SKF | TBD | 1 | TBD | NEEDS ORDERING (idle whistle) | | | |
| 23 | Cowl / scuttle panel (فبرة المساحات) | via ePER | ⬜ | genuine or clean used (plastic — Estirad OK) | Genuine/used | TBD | 1 | TBD | **NEEDS ORDERING — CONFIRMED broken (photo)** | | | |
| 24 | Radiator (TURBO version) | 51780659 | ⬜ | Valeo · Denso · Behr · Nissens | TBD | TBD | 0–1 | TBD | ON HOLD (pressure test) | | | |
| 25 | Complete driveshaft RH / inner CV | ePER SGR.21220 item 1; boot clamp item 2 | ⬜ | GKN/Löbro · SKF · Metelli | TBD | TBD | 0–1 | TBD | ON HOLD (load-noise tests) | | | |
| 26 | Rear engine mount (torque rod) | 51782714 | ⬜ | Corteco · Febi · Magneti Marelli | TBD | TBD | 0–1 | TBD | ON HOLD (load-noise tests) | | | |
| 27 | Airbag / BSI / ESP module | — | — | — | — | — | 0 | — | **DO NOT BUY** — scans show intermittent CAN = wiring, not module | | | |
| 28 | Rear harness repair (impact zone) | auto-electric labor | — | solder+heatshrink · plate-light short B1023 | — | TBD | 1 | labor | **NEEDS REPAIR — CONFIRMED (post-impact CAN fault)** | | | |
| 29 | Battery ~60Ah 540CCA (weak — scan-confirmed) | — | — | Varta · Bosch | TBD | TBD | 1 | TBD | **NEEDS ORDERING — weak (12.0V/10.3V cranking)** | | | |
| 30 | Number-plate light unit/pins (if damaged) | via ePER | ⬜ | genuine/used | TBD | TBD | 0–1 | cheap | NEEDS CHECK (B1023 short) | | | |

## Installed (from verified history — see [`08-service-history.md`](08-service-history.md))

| Part | Installed | Km | Paid (EGP) |
|---|---|---|---|
| Front suspension overhaul (shocks/mounts/links/arms/inner rod) | Sep 2021 | 102,000 | 4,195 |
| Timing belt kit + acc. belt/tensioner + water pump + plugs | Oct 2023 | 112,000 | 5,400 |
| Oil separator (PCV) | ~Oct 2023 ❓ | ~122,000 | 1,000 |
| Fuel pump | — | 115,000 | 2,100 |
| Clutch LuK + release bearing + 2 engine mounts | — | 125,000 | 16,200 |
| Alternator | — | 128,000 | 3,300 |
| Rear brake pads | Nov 2025 | 135,000 | ~600–900 |
| Flywheel + front discs/pads + steering rack | Dec 2025 | 140,000 | 15,800+ |
| Gearbox oil + brake fluid change | with clutch job | 125,000 | incl. |

(Turbo + exhaust manifold: **original, never changed** — earlier ledger figures were quotes.)

## Rules

1. Nothing moves from `ON HOLD` to `NEEDS ORDERING` without a diagnosis note in the relevant job card.
2. Every purchase: record invoice photo reference, supplier, warranty terms, and update [`04-purchase-tracker.md`](04-purchase-tracker.md).
3. Counterfeit alert (Egypt): verify Gates/SKF/NGK/Bosch anti-fake QR codes; buy from authorized distributors where possible.
4. Estirad (imported used) is acceptable ONLY for: body panels, interior trim, alternator/starter cores, EPS column (tested) — **never** rubber, belts, brake friction, bearings, or filters.
